...
interface EyeExpressionSignalProps {
    timeFilter: string
}

const EyeExpressionSignal = ({ timeFilter }: EyeExpressionSignalProps) => {
    const generateEyeData = (filter: string) => {
        const now = new Date()
        const data = []
        const labels = []
        ...
        for (let i = points - 1; i >= 0; i--) {
            ...
            const baseEye = 70
            const variation = Math.sin(i * 0.6) * 20 + Math.random() * 12
            data.push(Math.max(30, Math.min(100, baseEye + variation)))
        }
        
        return { data, labels }
    }

    const { data, labels } = generateEyeData(timeFilter)
...
export default EyeExpressionSignal 