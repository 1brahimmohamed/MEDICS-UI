...
interface HeartRateVariabilitySignalProps {
    timeFilter: string
}

const HeartRateVariabilitySignal = ({ timeFilter }: HeartRateVariabilitySignalProps) => {
    const generateHRVData = (filter: string) => {
        const now = new Date()
        const data = []
        const labels = []
        ...
        for (let i = points - 1; i >= 0; i--) {
            ...
            const baseHRV = 45
            const variation = Math.sin(i * 0.4) * 20 + Math.random() * 10
            data.push(Math.max(20, Math.min(80, baseHRV + variation)))
        }
        
        return { data, labels }
    }

    const { data, labels } = generateHRVData(timeFilter)
...
export default HeartRateVariabilitySignal 