...
interface FacialExpressionSignalProps {
    timeFilter: string
}

const FacialExpressionSignal = ({ timeFilter }: FacialExpressionSignalProps) => {
    const generateFacialData = (filter: string) => {
        const now = new Date()
        const data = []
        const labels = []
        ...
        for (let i = points - 1; i >= 0; i--) {
            ...
            const baseExpression = 60
            const variation = Math.sin(i * 0.7) * 25 + Math.random() * 15
            data.push(Math.max(20, Math.min(100, baseExpression + variation)))
        }
        
        return { data, labels }
    }

    const { data, labels } = generateFacialData(timeFilter)
...
export default FacialExpressionSignal 