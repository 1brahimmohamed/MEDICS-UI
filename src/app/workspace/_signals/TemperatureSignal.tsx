...
interface TemperatureSignalProps {
    timeFilter: string
}

const TemperatureSignal = ({ timeFilter }: TemperatureSignalProps) => {
    const generateTemperatureData = (filter: string) => {
        const now = new Date()
        const data = []
        const labels = []
        ...
        for (let i = points - 1; i >= 0; i--) {
            ...
            const baseTemp = 37.0
            const variation = Math.sin(i * 0.3) * 0.8 + Math.random() * 0.4
            data.push(Math.max(36.0, Math.min(38.5, baseTemp + variation)))
        }
        
        return { data, labels }
    }

    const { data, labels } = generateTemperatureData(timeFilter)
...
export default TemperatureSignal 