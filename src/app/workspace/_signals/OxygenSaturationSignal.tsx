...
interface OxygenSaturationSignalProps {
    timeFilter: string
}

const OxygenSaturationSignal = ({ timeFilter }: OxygenSaturationSignalProps) => {
    const generateOxygenData = (filter: string) => {
        const now = new Date()
        const data = []
        const labels = []
        ...
        for (let i = points - 1; i >= 0; i--) {
            ...
            const baseO2 = 98
            const variation = Math.sin(i * 0.2) * 1.5 + Math.random() * 1
            data.push(Math.max(95, Math.min(100, baseO2 + variation)))
        }
        
        return { data, labels }
    }

    const { data, labels } = generateOxygenData(timeFilter)
...
export default OxygenSaturationSignal 